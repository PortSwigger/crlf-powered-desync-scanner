...
class WAFChecker {
    ...
    init {
        wafSignatures.add(WAFSignature(
            name = "akamai",
            techniques = listOf("dupeHost", "missingHost", "clNoHost", "clTimeout"),
	...
	wafSignatures.add(WAFSignature(
            name = "underAttack",
            techniques = listOf("header|javascript"),
            bodyMatches = listOf("YThis website is using a security service to protect itself from online attacks. The action you just performed triggered the security solution. There are several actions that could trigger this block including submitting a certain word or phrase, a SQL command or malformed data."),
	    serverMatches = listOf(),
	    isAndMatch = true
        ))
    ...
    fun isWafResponse(response: HttpResponse?, technique: String): Boolean {
	if (response == null) {
	    return false
	}
        for (signature in wafSignatures) {
